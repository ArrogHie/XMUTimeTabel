...
<h1 align="center">厦大课表（Sleepy · 厦门大学专属版）</h1>

<p align="center">
  A clean, Material You Android schedule/timetable app built with Kotlin + Jetpack Compose.<br>
  Material You 设计的纯净 Android 课程表 App — 厦门大学教务导入 · 多视图 · 桌面小组件 · HSV 自定义配色
</p>
...
<p align="center">
  <a href="README.md">中文</a> · <a href="README_EN.md">English</a> · <a href="https://github.com/lingion/sleepy/releases">Download APK</a>
</p>

---

> **Keywords (SEO):** Android schedule app, 课程表, 课表, timetable, university schedule, Jetpack Compose, Material You, 厦门大学, XMU, Xiamen University, jw.xmu.edu.cn, home screen widget, HSV color picker, 开源课表, 开源课程表
...
Sleepy 厦门大学专属版乃 Android 课程表工具。主旨：**轻、快、准**。教务直连专为厦门大学设计（统一身份认证手动登录 / 账号密码自动登录双通道），另支持多格式解析、五类桌面 Widget、每日课程通知、深色模式和多套主题配色。

### v1.0.51-xmu（厦门大学专属版）

- 教务直连仅保留厦门大学：移除其余 180 所学校目录与全部多协议解析器，改为双通道（WebView 手动登录 / 账号密码自动登录），按金智 gsapp/wdkbapp 接口时序抓取课表并落库。
- 应用名改「厦大课表」。
...
本版教务直连仅面向厦门大学（统一身份认证 + 金智 gsapp/wdkbapp 微服务课表接口），提供两种方式：

| 方式 | 流程 |
|---|---|
| **手动登录** | 应用内打开厦大统一身份认证页面 → 亲自输入学号/密码/验证码登录 → 回到教务门户后输入学号并点「导入课表」→ 自动抓取课表 → 预览 → 导入 |
| **账号密码自动登录** | 输入学号与密码 → 应用按教务登录流程（AES 加密）自动登录并抓取课表 → 预览 → 导入（密码仅当次使用，不保存；教务启用图形验证码时请改用手动登录） |
...
| 入口 | 流程 |
|---|---|
| **教务直连** | 选择「手动登录 / 账号密码自动登录」→ 抓取课表 → 预览 → 导入 |
| **粘贴课表文本** | 粘贴任意格式文本 → 自动识别格式 → 预览 → 导入 |
| **从文件导入** | 选择 `.json` / `.ics` / `.csv` / `.html` 文件 → 预览 → 导入 |

> 其它学校不再收录。如需恢复多学校支持，请基于上游通用版（WakeUp / WakeupSchedule_BUPT 协议族）另行构建。
...
| **开源声明** | GPL-3.0 协议说明、上游参考项目（WakeUp 系 3 个跨校普适项目）的许可证和参考范围 |
...
│   │   │   │   ├── jw/                   # 厦门大学教务直连 (JwXmuParser + XmuAutoLoginClient +
│   │   │   │   │                         # XmuJw + JwImportViewModel + JwParserRegistry)
...
│   │   │   │   │   ├── today/            # 今日视图
│   │   │   │   │   ├── edit/             # 课程编辑
│   │   │   │   │   ├── imports/          # 厦大教务导入 (JwImportActivity + WebView) + 文本/文件导入
│   │   │   │   │   ├── manage/           # 课程管理
